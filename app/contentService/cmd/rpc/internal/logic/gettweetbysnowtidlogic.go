...
func (l *GetTweetBySnowTidLogic) GetTweetBySnowTid(in *pb.GetTweetBySnowTidReq) (*pb.GetTweetBySnowTidResp, error) {
	...
	if err != nil {
		logx.Infof("缓存未命中: tweet:%d, err:%v", in.SnowTid, err)
		// 2. 查数据库
		...
		go func() {
			if err := l.svcCtx.SetTweetToCache(context.Background(), in.SnowTid, tweet); err != nil {
				logx.Errorf("SetTweetToCache error, snowTid:%d, err:%v", in.SnowTid, err)
			}
		}()
	} else {
		logx.Infof("缓存命中: tweet:%d", in.SnowTid)
	}
	...
	if err != nil {
		logx.Errorf("GetTweetBySnowTid GetUserInfo error, uid:%d, err:%v", tweet.Uid, err)
